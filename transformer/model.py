...
class LayerNormalization(nn.Module):
  def __init__(self, eps: float = -1e6):
    super().__init__()
    self.eps = eps
...
class MultiHeadAttentionBlock(nn.Module):
  ...
  def attention(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
        mask: torch.Tensor, dropout: nn.Dropout) -> tuple[torch.Tensor, torch.Tensor]:
    d_k = query.shape[-1]
  ...
  def forward(self, q, k, v, mask) -> torch.Tensor:
    query: torch.Tensor = self.w_q(q) # (b, seq_len, d_model)
    key: torch.Tensor = self.w_k(k) # (b, seq_len, d_model)
    value: torch.Tensor = self.w_v(v) # (b, seq_len, d_model)
...
class Encoder(nn.Module):
  def __init__(self, layers: nn.ModuleList[EncoderBlock]):
    super().__init__()
    self.layers = layers
    self.norm = LayerNormalization()
...
class Decoder(nn.Module):
  def __init__(self, layers: nn.ModuleList[DecoderBlock]):
    super().__init__()
    self.layers = layers
    self.norm = LayerNormalization()
